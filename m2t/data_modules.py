...
def preprocess_multimodal_mappable(e: Dict[str, Any], multimodal_cfg: Dict[str, Any]):
    ...
    # fixed: use 1 replace token for audio (clap)
    # audio_token_len = 1
    ...
    for sentence in source:
        ...
        # add tokens to indicate position of audio encoding
...
def combine_emb_instruct_data(input_dir):
    """
    combine embedding data and instruct data.
    return a list.
    """
    ...
    src = []
    # urls = [os.path.join(input_dir, fp) for fp in os.listdir(input_dir)]
    for url in input_dir:
        emb_file = list_files_with_extension(url, '.npy')
        instruct_file = list_files_with_extension(url, 'jsonl')
        # load files
        assert len(emb_file)==1, 'multiple emb files'
        assert len(instruct_file)==1, 'multiple instruct files'
        emb_data = np.load(emb_file[0])
        with open(instruct_file[0], 'rb') as f:
            instruct_data = json.load(f)

        json_data=dict()
        json_data["response"] = [{"question": prompt_text, "answer": instruct_data['response']['caption']}]

        elem=dict()

        if len(emb_data.shape)>1:
            emb_data = emb_data[0]

        elem["audio_encoding.pyd"] = emb_data
        elem["__key__"] = instruct_data['uri']
...
def read_combined_data(urls, multimodal_cfg, tokenizer):
    ...
    mm2 = map(_preprocess_for_lm, mm1)

    return mm2


def make_data_module_local(
    tokenizer: transformers.PreTrainedTokenizer,
    data_args,
    ...
    if data_args.train_data_path:
        train_dataset = read_webdataset_local(
    ...
    if data_args.eval_data_path:
        eval_dataset = read_webdataset_local(
...
def deserialize_pkl_from_bytes(byte_content):
    """
    Deserializes a pickle object from a byte stream.

    Parameters:
    - byte_content: bytes, The byte stream containing the serialized pickle object.
    ...
    try:
        ...
        obj = pickle.loads(byte_content)
        return obj
    except pickle.PickleError as e:
        # Handle deserialization errors (optional)
        print(f"Error deserializing object: {e}")
        return None

# Define a processing function that will deserialize the `.pkl` files
def process_pkl_sample(sample):
    if "pkl" in sample:
        # Assuming `.pkl` files are identified by a 'pkl' key in the sample dict
        pkl_data = sample['pkl']
        sample = deserialize_pkl_from_bytes(pkl_data)

        return sample


def read_webdataset_local(
    url: str,
    multimodal_cfg: Dict[str, Any],
    tokenizer,
) -> wds.WebDataset:
    ...
    # methods via a custom IterableDataset class, refer to https://webdataset.github.io/webdataset/sources/
    logging.warning(f"reading datasets from {url}")
    urls = list_files_with_extension(url, 'tar')
