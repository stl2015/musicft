...
def gen_audio_embeddings(
        input_dir='datasets/musicnet_single/', 
        output_dir='tmp/clap-infer', 
        ckpt_file='music_audioset_epoch_15_esc_90.14.pt',
        ):
    ...
    input_paths = list_files_with_extension(input_dir, extension=".wav")
    print(f"[INFO] processing files {input_paths}")

    if not os.path.exists(ckpt_file):
        import wget
        download_link = 'https://huggingface.co/lukewys/laion_clap/resolve/main/'
        download_names = 'music_audioset_epoch_15_esc_90.14.pt'
        ckpt_file = wget.download(download_link + download_names, '')

    clap_model_handler = ClapModelHandler(ckpt_file=ckpt_file)
    ...
    with beam.Pipeline(options=pipeline_options) as p:
        p = (
            ...
            | "FilterInvalidOutputs" >> beam.Filter(lambda x: x.inference is not None)
            # | beam.Map(print)
            | "WriteOutput" >> beam.Map(write_output, output_dir=output_dir)
        )

    return True


def gen_audio_embeddings_app(
