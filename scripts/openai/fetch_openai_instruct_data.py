...
def prompt(model: str, uri: str, metadata, prompt_helper: PromptHelper) -> Dict[str, List[str]]:
    ...
    for retry in range(NUM_RETRIES):
        try:
            response_iterator = client.chat.completions.create(model=model,
            messages=messages,
            stream=True,
            timeout=60)
            response_chunks = []
...
def main():
    ...
    parser.add_argument(
        ...
        choices=["gpt-4", "gpt-4-1106-preview", "gpt-3.5-turbo", "gpt-3.5-turbo-16k"],
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--prompt-type",
        default="default",
        choices=["default", "mir", "reasoning", "captioning", "notes_captioning"],
